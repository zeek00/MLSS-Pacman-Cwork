...
namespace Pacman.GameLogic.Ghosts
{
	...
	public abstract class Ghost : Entity, ICloneable
	{
		...
		public void MTS(Entity entity)
		{

			List<Direction> possible = PossibleDirections();


			float upMove, downMove, leftMove, rightMove;

			upMove = (float)Math.Sqrt(Math.Pow(X - entity.X, 2) + Math.Pow(Y - 1.0f - entity.Y, 2));
			downMove = (float)Math.Sqrt(Math.Pow(X - entity.X, 2) + Math.Pow(Y + 1.0f - entity.Y, 2));
			leftMove = (float)Math.Sqrt(Math.Pow(X - 1.0f - entity.X, 2) + Math.Pow(Y - entity.Y, 2));
			rightMove = (float)Math.Sqrt(Math.Pow(X + 1.0f - entity.X, 2) + Math.Pow(Y - entity.Y, 2));
			float gMin = Math.Min(upMove, Math.Min(downMove, Math.Min(leftMove, rightMove)));

			if (possible.Contains(Direction.Up))
			{
				if (gMin.Equals(upMove))
				{
